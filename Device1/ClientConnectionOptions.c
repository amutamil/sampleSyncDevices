...
bool isConnectionSSLWSS(char* connection)
{
	if ((isConnectionIsSSL(connection) || isConnectionIsWSS(connection)))
		return true;
	return false;
}
bool isConnectionIsSSLorWSS(struct pubsub_opts *opts)
{
	if (opts->connection && isConnectionSSLWSS(opts->connection))
		return true;
	return false;
}