...
int main(int argc, char** argv)
{
	...
	if (argc < 2)                 //oly executable file given with no command display the usage of the product
		usage(&opts, (pubsub_opts_nameValue*)infos, program_name);

	if (getopts(argc, argv, &opts) != 0)   //error in commands again output the usage of the product
		usage(&opts, (pubsub_opts_nameValue*)infos, program_name);

	if (opts.connection)     //check for the connection specified for wss or ssl
		url = opts.connection;
	else
	{
		url = malloc(100);
		sprintf(url, "%s:%s", opts.host, opts.port); //if not make the socket address
	}
	if (opts.verbose)
		printf("URL is %s\n", url);
